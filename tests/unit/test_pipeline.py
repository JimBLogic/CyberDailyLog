...
def window(): return datetime(2026,7,14,tzinfo=timezone.utc), datetime(2026,7,16,tzinfo=timezone.utc)

def test_collectors_cover_required_cases():
    s,u=window(); n,h=NvdCollector(offline=True).collect(s,u)
    assert h.items_received==3 and h.items_rejected==1
    assert any(i.cvss_score is None for i in n)
    k,kh=CisaKevCollector(offline=True).collect(s,u); assert k[0].known_exploited is True and kh.status=='fixture_only'
    g,gh=GitHubAdvisoryCollector(offline=True).collect(s,u); assert any(i.withdrawn for i in g) and gh.items_received==2
...
def test_correlation_provenance_and_no_invented_fixed_versions():
    ...
    for cls in (CisaKevCollector,NvdCollector,GitHubAdvisoryCollector):
        got,_=cls(offline=True).collect(s,u); items+=got
    merged=merge_items(items); one=[i for i in merged if 'CVE-2099-0001' in i.cve_ids][0]
...
def test_scoring_kev_outranks_high_cvss_and_reasons():
    ...
    for cls in (CisaKevCollector,NvdCollector,GitHubAdvisoryCollector):
        got,_=cls(offline=True).collect(s,u); items+=got
    scored=score_items(merge_items(items), load_yaml(pathlib.Path('config/scoring.yml')), load_yaml(pathlib.Path('config/technologies.yml')), s,u)
...
def test_offline_generation_and_schema(tmp_path):
    r=Pipeline(output_dir=tmp_path, offline=True).run(*window())
