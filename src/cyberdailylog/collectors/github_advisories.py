...
class GitHubAdvisoryCollector(BaseCollector):
    ...
    def _item(self, a):
        ...
        item = IntelligenceItem(
            ...
            fixed_versions=[v.get("patched_versions", "") for v in vulns if v.get("patched_versions")],
            severity=a.get("severity"),
            cvss_score=(a.get("cvss") or {}).get("score"),
            cvss_vector=(a.get("cvss") or {}).get("vector_string"),
    ...
    def collect(self, since, until):
        ...
        try:
            pages = (
                [self.fixture_json("github_advisories_page1.json"), self.fixture_json("github_advisories_page2.json")]
                if self.offline
                else [
                    self.http.get(
                        self.endpoint,
            ...
            for data in pages:
                for a in data:
                    rec += 1
                    item = self._item(a)
                    (items.append(item) if not item.withdrawn else (items.append(item), None))
