...
class GitHubReleaseCollector(BaseCollector):
    ...
    def collect(self, since, until):
        ...
        try:
            data = (
                self.fixture_json("github_releases.json")
                if self.offline
                else {
                    r: self.http.get(
                        f"https://api.github.com/repos/{r}/releases",
                    ...
                    if since <= dt <= until:
                        items.append(
                            IntelligenceItem(
                                ...
                                published_at=dt,
                                modified_at=dt,
                                references=[rel["html_url"]],
                                blue_team_relevance="Allowlisted defensive project release; review release notes for detection or monitoring updates.",
                                detection_opportunities=["Review release notes for defensive content changes."],
                                confidence="medium",
                            )
                        )
            return items, self.timed_health("fixture_only" if self.offline else "healthy", started, rec, len(items))
        except Exception as e:
            return items, self.timed_health("degraded", started, rec, len(items), err=e)
