...
def main(argv=None):
    ...
    if a.cmd in {"run", "collect", "generate", "source-health"}:
        r = Pipeline(Path(a.config_dir), Path(a.output_dir), a.offline_fixtures).run(
            dt(a.since) if a.since else None,
            dt(a.until) if a.until else None,
            a.lookback_hours,
            a.dry_run,
            a.fail_on_degraded,
        )
        if a.cmd == "source-health":
            print(json.dumps([h.to_dict() for h in r.source_health], indent=2))
        else:
            print(f"Generated {len(r.items)} selected items; degraded={r.degraded}")
    elif a.cmd == "validate":
